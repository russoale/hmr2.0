...
class SmplWidget(Qt5PygletWidget):
    ...
    def on_mouse_double_click(self, x, y):
        ...
        if len(indices) > 0 and len(indices[0]) > 0:
            ...
            # ray_visualize = load_path(np.hstack((
            #     ray_origin,
            #     ray_origin + ray_direction)).reshape(-1, 2, 3))
            # self._scene.add_geometry(ray_visualize)

            # path = np.hstack((locations[idx].reshape([1, 3]), locations[idx] + ray_direction)).reshape(-1, 2, 3)
